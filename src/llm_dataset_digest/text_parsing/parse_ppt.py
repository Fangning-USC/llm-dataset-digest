...
def parse_ppt(  # noqa
    ...
    """This function parse and save text of a PowerPoint separately by text boxes.

    Args:
        ppt_file: file object.
        ppt_path: Path of the PowerPoint.
        ppt_path: Path to store the json files.
    ...
    Returns:
        json files.
    """
    file_name = str(ppt_file).split(".")[0]
    folder_name = ppt_path + "\\" + file_name

    if not os.path.exists(folder_name):  # noqa
        os.mkdir(folder_name)  # noqa

    prs = Presentation(ppt_path + "\\" + str(ppt_file))
    output_file = ""

    for i, slide in enumerate(prs.slides):  # noqa
        output_page = ""
        output_text_page = ""
        ...
        if output_text_page.strip():
            ...
            save_to_json(path=storage_path, item=dict_shape, item_name=item_name)
